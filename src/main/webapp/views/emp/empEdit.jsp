...
<body>
<form action="/empEdit.do" method="post" onsubmit="return validateForm()" enctype="multipart/form-data">
    <div class="parent">
        <div class="sidebar">
        ...
        <div class="content">
            ...
            <div class="form-group">
                ...
                <input type="text"  name = "post_no" class="form-control" id="sample6_postcode" placeholder="우편번호" value = "${emp.post_no}" required>
                ...
                <input type="text" class="form-control" id="phone" name="phone" value = "${emp.phone}" required>
            </div>
            <div class="form-group">
                <span class="subject">주소</span>
                <input class="form-control" name = "address" type="text" id="sample6_address" value ="${emp.address}" placeholder="주소" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control" name = "detail_address" type="text" id="sample6_extraAddress" value = "${emp.detail_address}" placeholder="상세주소" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control" id="bank_name" name="bank_name" value = "${emp.bank_name}" required>
                    <option value=""></option>
                    <option value="하나은행"<c:if test="${emp.bank_name == '하나은행'}">selected</c:if>>하나은행</option>
                    <option value="신한은행"<c:if test="${emp.bank_name == '신한은행'}">selected</c:if>>신한은행</option>
                    <option value="국민은행"<c:if test="${emp.bank_name == '국민은행'}">selected</c:if>>국민은행</option>
                    <option value="카카오뱅크"<c:if test="${emp.bank_name == '카카오뱅크'}">selected</c:if>>카카오뱅크</option>
                    <option value="토스뱅크"<c:if test="${emp.bank_name == '토스뱅크'}">selected</c:if>>토스뱅크</option>
                    <option value="농협은행"<c:if test="${emp.bank_name == '농협은행'}">selected</c:if>>농협은행</option>
                    <option value="기업은행"<c:if test="${emp.bank_name == '기업은행'}">selected</c:if>>기업은행</option>                    
                </select>
                <span class="subject">권한</span>
                <select class="form-control" id="authority" name="authority" value ="${emp.authority}" required>
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="account_no" name="account_no" value = "${emp.account_no}" required>
                <span class="subject">입사일</span>
    	<input type="date" class="form-control" id="join_date" name="join_date" value="${emp.join_date}" required>
            </div> 
            <br><br>
                <div class="btnParent">                             
